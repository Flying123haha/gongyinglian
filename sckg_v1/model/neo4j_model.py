...
class Neo4j():
    ...
    def connectDB(self):
        self.graph = Graph("http://0.0.0.0:7473", auth=("neo4j","123"))
        print("Neo4J已连接...")
    ...
    def getQ2Cql(self):
        ...
        self.q2s = Question2Cql.Question2Cql(['/data1/flying/code/aistudy/sckg/sckg_v1/model/qa_demo/external_dict/gongyinglian.txt'])
    ...
    def findNewsByEntity(self, entity1):
        answer = self.graph.run("MATCH (n1 {name:\"" + str(entity1) + "\"})- [rel] -> (n2:news) RETURN n1,rel,n2").data()
        # if(answer is None):
    ...
    def findOtherEntities2(self, entity, relation):
        if(relation=='竞争'): # 查询entity1-[竞争]->entity2的结果是一致的
            ...
            if(relation=='客户'): # 这里的逻辑是entity1-[supply]->entity 谁的客户是entity 即谁供应entity
                relation='供应'
            answer = self.graph.run("MATCH (entity1)- [relation {type:\"" + str(relation) + "\"}] -> (entity2 {name:\"" + str(
    ...
    def findEntityRelation(self, entity1, relation, entity2):
        ...
        answer = self.graph.run("MATCH (entity1:HudongItem {title:\"" + str(entity1) + "\"})- [relation:RELATION {type:\"" + str(
        ...
        if (answer is None):
            answer = self.graph.run("MATCH (entity1:NewNode {title:\"" + str(entity1) + "\"})- [relation:RELATION {type:\"" + str(
                relation) + "\"}] -> (entity2:HudongItem{title:\"" + entity2 + "\"}) RETURN entity1,relation,entity2").data()
        if (answer is None):
            answer = self.graph.run("MATCH (entity1:NewNode {title:\"" + str(entity1) + "\"})- [relation:RELATION {type:\"" + str(
                relation) + "\"}] -> (entity2:NewNode{title:\"" + entity2 + "\"}) RETURN entity1,relation,entity2").data()
